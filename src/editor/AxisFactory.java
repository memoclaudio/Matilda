...
public class AxisFactory {
	...
	// static Logger LoggingSystem.getLogger(this) =
	// LoggingSystem.getLogger(TubeFactory.class);

	public double[][] theCurve, userTangents = null, userBinormals = null,
	...
	public boolean closedCurve = false, vertexColorsEnabled = false;
	 ...
	 * cross section of the tube. For flexibility: unless the first and last
	 * points are equal, the curve is considered to be open and the resulting
	 * tube will not close up. Default: an octagon lying on the unit circle.
	 * 
	 * @param crossSection
	 */
	 ...
	 * {@link TubeUtility#PARALLEL} or {@link TubeUtility#FRENET} displacement?
	 * Default: {@link TubeUtility#PARALLEL}
	 * 
	...
	/**
	 * Set the radius of the tube. To be exact, this is applied as a scale
	 * factor to the cross section before it is swept out to make the tube.
	 * 
	 * @param radius
	 */
	 ...
	 * swept along the curve. This twisting is done proportional to the arc
	 * length of the curve. For creating exotic shapes. Default: 0
	 * 
	 * @param twists
	 */
	...
	/**
	 * Whether to apply vertex colors to the output tube. See
	 * {@link #setVertexColors(double[][])}.
	 * 
	 ...
	 * 
	 * @param edgeColors
	 *            double[n][] where n is number of segments in curve
	 */
	 ...
	 * 
	 * @param vertexColors
	 *            double[n][] where n is number of vertices in curve
	 */
	...
	/**
	 * Whether the output geometry should have automatic texture coordinates. If
	 * true, texture coordinates will be based on the tubing parameters: u is
	 * the cross section parameter, v is the length of the tube.
	 * 
	 * @return
	...
	/**
	 * If true, force the generated v- texture coordinates to run from 0 to 1
	 * proportional to the (discrete) arc length of the curve.
	 * 
	 * @param arcLengthTextureCoordinates
	 */
	public void setArcLengthTextureCoordinates(
	...
	public FrameInfo[] makeFrameField(double[][] polygon, FrameFieldType type,
			int metric) {
		if (frames != null && !framesDirty)
			return frames;
		...
		if ((debug & 1) != 0)
			LoggingSystem.getLogger(this).log(Level.FINER,
					"Generating frame field for metric " + metric);
		...
		double[] d = new double[numberJoints - 2]; // distances between adjacent
													// points
		if ((debug & 32) != 0) {
			for (int i = 0; i < numberJoints; ++i) {
				LoggingSystem.getLogger(this).log(Level.FINER,
						"Vertex " + i + " : " + Rn.toString(polygonh[i]));
			}
		}
		double[] frame = new double[16];
		double totalLength = 0.0;
		for (int i = 1; i < numberJoints - 1; ++i) {
			d[i - 1] = (totalLength += Pn.distanceBetween(polygonh[i - 1],
					polygonh[i], metric));
		}
		...
		for (int i = 1; i < numberJoints - 1; ++i) {
			...
			if ((debug & 2) != 0)
				LoggingSystem.getLogger(this).log(Level.FINER,
						"Polar plane is: " + Rn.toString(polarPlane));

			double[] osculatingPlane = P3.planeFromPoints(null,
					polygonh[i - 1], polygonh[i], polygonh[i + 1]);
			...
			if (size < 10E-16) { // collinear points!
				...
				if ((debug & 2) != 0)
					LoggingSystem.getLogger(this).log(Level.FINER,
							"degenerate binormal");
				if (i == 1)
					binormalField[i - 1] = getInitialBinormal(polygonh, metric);
				else
					Pn.projectToTangentSpace(binormalField[i - 1], polygonh[i],
							binormalField[i - 2], metric);
			} else
				Pn.polarizePlane(binormalField[i - 1], osculatingPlane, metric);
			if (userBinormals != null)
				System.arraycopy(userBinormals[i - 1], 0, binormalField[i - 1],
						0, userBinormals[i - 1].length);
			Pn.setToLength(binormalField[i - 1], binormalField[i - 1], 1.0,
					metric);
			if (i > 1 && metric == Pn.ELLIPTIC) {
				double foo = Pn.angleBetween(binormalField[i - 2],
						binormalField[i - 1], metric);
				if (Math.abs(foo) > Math.PI / 2)
					Rn.times(binormalField[i - 1], -1, binormalField[i - 1]);
			}
			if ((debug & 2) != 0)
				LoggingSystem.getLogger(this).log(Level.FINER,
						"Binormal is " + Rn.toString(binormalField[i - 1]));
			// System.err.println("Binormal field = "+Rn.toString(binormalField[i-1]));

			/*
			 * Next try to calculate the tangent as a "mid-plane" if the three
			 * points are not collinear
			 */
			double[] midPlane = null, plane1 = null, plane2 = null;
			if (!collinear) {
				plane1 = P3.planeFromPoints(null, binormalField[i - 1],
						polygonh[i], polygonh[i - 1]);
				plane2 = P3.planeFromPoints(null, binormalField[i - 1],
						polygonh[i], polygonh[i + 1]);
				midPlane = Pn.midPlane(null, plane1, plane2, metric);
				size = Rn.euclideanNormSquared(midPlane);
				if ((debug & 2) != 0)
					LoggingSystem.getLogger(this).log(Level.FINER,
							"tangent norm squared is " + size);
				theta = Pn.angleBetween(plane1, plane2, metric);
			}
			/*
			 * if this is degenerate, then the curve must be collinear at this
			 * node get the tangent by projecting the line into the tangent
			 * space at this point
			 */
			if (collinear || size < 10E-16) {
				...
				if ((debug & 2) != 0)
					LoggingSystem.getLogger(this).log(Level.FINER,
							"degenerate Tangent vector");
				// if the two vertices in the following call are swapped
				double[] pseudoT = P3.lineIntersectPlane(null, polygonh[i - 1],
						polygonh[i + 1], polarPlane);
				if ((debug & 2) != 0)
					LoggingSystem.getLogger(this).log(Level.FINE,
							"pseudo-Tangent vector is " + Rn.toString(pseudoT));
				...
				if (metric != Pn.EUCLIDEAN) {
					...
					// has to be flipped in this case but not in the
					// non-euclidean case
					midPlane = Rn.times(null, -1.0, pseudoT);
					// midPlane = pseudoT;
					// the euclidean polar of a point is the plane at infinity:
					// we want something
					// much more specific:
					// we assume the polygonal data is dehomogenized (last coord
					// = 1)
			...
			if ((debug & 2) != 0)
				LoggingSystem.getLogger(this).log(Level.FINE,
						"Midplane is " + Rn.toString(midPlane));
			Pn.polarizePlane(tangentField[i - 1], midPlane, metric);
			if (userTangents == null) {
			} else {
				System.arraycopy(userTangents[i - 1], 0, tangentField[i - 1],
						0, userTangents[i - 1].length);
				midPlane = Rn.planeParallelToPassingThrough(null,
						userTangents[i - 1], polygonh[i]);
				// System.err.println("given midplane is "+Rn.toString(midPlane));
				if (i > 1)
					theta = Pn.angleBetween(userTangents[i - 2],
							userTangents[i - 1], metric);
				else
					theta = 0;
			}
			// This is a hack to try to choose the correct version of the
			// tangent vector:
			// since we're in projective space, t and -t are equivalent but only
			// one
			...
			Pn.setToLength(tangentField[i - 1], tangentField[i - 1], 1.0,
					metric);
			...
			Pn.polarizePlane(frenetNormalField[i - 1], P3.planeFromPoints(null,
					binormalField[i - 1], tangentField[i - 1], polygonh[i]),
					metric);
			Pn.setToLength(frenetNormalField[i - 1], frenetNormalField[i - 1],
					1.0, metric);
			if ((debug & 2) != 0)
				LoggingSystem.getLogger(this).log(
						Level.FINE,
						"frenet normal is "
								+ Rn.toString(frenetNormalField[i - 1]));
			if (type == FrameFieldType.PARALLEL) {
				// get started
				if (i == 1)
					System.arraycopy(frenetNormalField[0], 0,
							parallelNormalField[0], 0, 4);
				else {
					double[] nPlane = P3.planeFromPoints(null, polygonh[i],
							polygonh[i - 1], parallelNormalField[i - 2]);
					double[] projectedN = P3.pointFromPlanes(null, nPlane,
							midPlane, polarPlane);
					if (Rn.euclideanNormSquared(projectedN) < 10E-16) {
						LoggingSystem.getLogger(this).log(Level.FINE,
								"degenerate normal");
						projectedN = parallelNormalField[i - 2]; // try
																	// something!
					}
					parallelNormalField[i - 1] = Pn.normalizePlane(null,
							projectedN, metric);
					if ((debug & 128) != 0)
						LoggingSystem
								.getLogger(this)
								.log(Level.FINE,
										"Parallel normal is "
												+ Rn.toString(parallelNormalField[i - 1]));

				}
				// if (size < 10E-16) {
				// if ((debug & 2) != 0)
				// LoggingSystem.getLogger(this).log(Level.FINE,"degenerate parallel normal");
				// if (i > 1) parallelNormalField[i-1] =
				// parallelNormalField[i-2];
				// }
				if (parallelNormalField[i - 1] == null) {
					...
					Pn.setToLength(parallelNormalField[i - 1],
							parallelNormalField[i - 1], 1.0, metric);
				phi = Pn.angleBetween(frenetNormalField[i - 1],
						parallelNormalField[i - 1], metric);
				...
				double a = Pn.angleBetween(parallelNormalField[i - 1],
						binormalField[i - 1], metric);
				if (a > Math.PI / 2)
					phi = -phi;
			}
			...
			frameInfo[i - 1] = new FrameInfo(Rn.transpose(null, frame),
					d[i - 1], theta, phi);
			if ((debug & 16) != 0)
				LoggingSystem.getLogger(this)
						.log(Level.FINE,
								"Frame " + (i - 1) + ": "
										+ frameInfo[i - 1].toString());
		}
		framesDirty = false;
		return frameInfo;
	}

	static double[] B = new double[] { Math.random(), Math.random(),
			Math.random(), 1.0 };
	...
	protected double[] getInitialBinormal(double[][] polygon, int metric) {
		...
		for (int i = 1; i < n - 1; ++i) {
			double[] bloop = Pn.polarize(null, P3.planeFromPoints(null,
					polygon[i - 1], polygon[i], polygon[i + 1]), metric);
		...
		// all points are collinear, choose a random plane through the first two
		// points and polarize it to get a point
		return Pn.polarizePlane(null, initialBinormal != null ? initialBinormal
				: P3.planeFromPoints(null, B, polygon[1], polygon[2]), metric);
	}

	static double[][] axes = { { 0, 0, 0 }, { 0.35, 0, 0 }, { 0, 0.35, 0 },
			{ 0, 0, 0.35 } };
	static int[][] axesIndices = { { 0, 1 }, { 0, 2 }, { 0, 3 } };
	static Color[] axesColors = { Color.red, Color.green, Color.blue };
	static String[] assi = { "X", "Y", "Z" };

	public static SceneGraphComponent getSceneGraphRepresentation(
			FrameInfo[] frames) {
		return getSceneGraphRepresentation(frames, .02);
	}

	public static SceneGraphComponent getSceneGraphRepresentation(
			FrameInfo[] frames, double scale) {
		...
		for (FrameInfo f : frames) {
			...
			double[] scaledFrame = Rn.times(null, f.frame,
					P3.makeRotationMatrixZ(null, f.phi));
		...
		ap.setAttribute(CommonAttributes.LINE_SHADER + "."
				+ CommonAttributes.TUBE_RADIUS, .001);
		ap.setAttribute(CommonAttributes.LINE_SHADER + "."
				+ CommonAttributes.TUBES_DRAW, false);
		ap.setAttribute(CommonAttributes.LINE_SHADER + "."
				+ CommonAttributes.DIFFUSE_COLOR, new Color(100, 200, 200));
		ap.setAttribute(CommonAttributes.LINE_SHADER + "."
				+ "polygonShader.diffuseColor", new Color(100, 200, 200));
	...
	public static SceneGraphComponent getXYZAxes() {
		...
		ils.setVertexAttributes(Attribute.LABELS,
