...
try {
    ...
    String read = null;
       while ((read = in.readLine()) != null) {
        ...
        try {if(splited.length==2 && Integer.parseInt(splited[0])>=0 && Integer.parseInt(splited[0])<10 && Integer.parseInt(splited[1])>=0 && Integer.parseInt(splited[1])<10)
          ...
          catch (NumberFormatException exc) {JOptionPane.showMessageDialog(null,
          		"File non valido, verificare il contenuto");
          while (t.getRowCount() > 0) {
             t.removeRow(0);
          }
...
p.getLineColorTransformation();
    ...
    public void setUpStringColumn(JTable table,
                                 TableColumn stringColumn) {
